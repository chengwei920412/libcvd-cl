...
template<class T>
void ransac(std::vector<Ransac<T> >& samples, int num_iterations, Hypothesis<T>& hyp, double& best_score, double inlier_threshold, Hypothesis<T>& prev_hyp){

  const int sample_size = T::sample_size;
  const int num_measurements = samples.size();
  ...
  for(int iter=0; iter<num_iterations; iter++){
    ...
    unsigned int sample_nos[T::sample_size];
